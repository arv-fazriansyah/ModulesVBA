function autoMergeFiles() {
  ...
  const templateIds = getTemplateId(); // Ambil template IDs dari sheet DATAUSER

  jobSettings.forEach((setting, index) => {
    // Gunakan template ID yang sesuai dengan indeks
    setting.templateId = templateIds[index] || setting.templateId;
    processJobSetting(activeSpreadsheet, setting);
  });
}

function getTemplateId() {
  const sheetId = '1_tj2qwnxDwtQDLzUVQtPHJidSS9cz5vMwTogsFDow5s'; // ID spreadsheet tujuan
  const dataSheet = SpreadsheetApp.openById(sheetId).getSheetByName('DATAUSER'); // Mengakses sheet DATAUSER
  const sourceSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('1. SEKOLAH'); // Mengakses sheet 1. SEKOLAH
  
  const searchValue = sourceSheet.getRange('J3').getValue().toString().trim(); // Mengambil nilai dari J3 dan menghapus spasi
  
  Logger.log('Nilai J3: ' + searchValue); // Menampilkan nilai J3 di log
  
  const dataRange = dataSheet.getRange('B2:B' + dataSheet.getLastRow()).getValues(); // Mengambil data dari B2:B
  
  let rowIndex = -1;
  
  // Mencari data yang cocok
  for (let i = 0; i < dataRange.length; i++) {
    if (dataRange[i][0].toString().trim() === searchValue) {
      rowIndex = i + 2; // Menyimpan baris yang ditemukan
      break;
    }
  }
  
  if (rowIndex === -1) {
    throw new Error('Data tidak ditemukan di kolom B.');
  }
  
  // Mengambil template ID sesuai baris yang ditemukan
  const templateIds = dataSheet.getRange(rowIndex, 5, 1, 3).getValues(); // Mengambil data dari kolom E hingga G
  
  Logger.log('Template ID ditemukan: ' + templateIds[0]); // Menampilkan template ID yang ditemukan
  
  return templateIds[0]; // Mengembalikan template ID yang ditemukan
}
